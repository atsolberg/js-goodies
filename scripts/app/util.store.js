...
util.store = (($) => {
   ...
   */
  mod.create = (name, initial) => {

    let state;
    let logging = globals.dev;
    let listeners = [];
    ...
    /** Retrieve any static app data the handler may provide. */
    const getData = () => util.isFunc(handler, 'getData') ? handler.getData() : null;
    ...
    return {
      name,
      getState, getData,
      subscribe, dispatch, change, select,
      toggleLogging
    };
  };
